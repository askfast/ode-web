define(['controllers/controllers'], function (controllers) {
  ...
  controllers.controller('groups', function ($rootScope, $scope, $location, data, Groups, Profile, $route, $routeParams, Storage, Slots, $timeout, Settings, Store) {
    ...
    function wisher(id) {
      ...
      Groups.wish(id).then(function (wish) {
        $scope.wished = true;
    ...
    $scope.searchMembers = function (query) {
      ...
      Groups.search(query).then(function (result) {
        if (result.error) {
    ...
    $scope.addMember = function (candidate) {
      ...
      Groups.addMember(candidate).then(function (result) {
        if (result.error) {
    ...
    $scope.removeMember = function (member, group) {
      ...
      Groups.removeMember(member, group).then(function (result) {
        if (result.error) {
    ...
    $scope.removeMembers = function (selection, group) {
      ...
      _.each(selection, function (value, user) {
        if (value)
          selected = true;
      });

      if (selected) {
        Groups.removeMembers(selection, group).then(function (result) {
          if (result.error) {
    ...
    $scope.groupSubmit = function (group) {
      ...
      Groups.save(group).then(function (returned) {
        if (returned.error) {
    ...
    $scope.userExists = function () {
      if ($scope.memberForm.username != '' || $scope.memberForm.username.length > 0) {
        ...
        $scope.checkUsername = setTimeout(function () {
          ...
          Profile.userExists($scope.memberForm.username).then(function (result) {
    ...
    $scope.memberSubmit = function (member) {
      ...
      if ($rootScope.phoneNumberParsed.result) {
        ...
        Profile.register(member).then(function (result) {
          if (result.error) {
    ...
    $scope.deleteGroup = function (id) {
      ...
      Groups.remove(id).then(function (result) {
        if (result.error) {
    ...
    // Not used in groups yet but login uses modal call. Fetch parent groups.
    $scope.fetchParent = function () {
      Groups.parents().then(function (result) {
        console.log('parent -> ', result);
      });
    };

    // Not used in groups yet. Fetch parent groups.
    $scope.fetchContainers = function (id) {
      Groups.containers(id).then(function (result) {
