define(['services/services'], function (services) {
  ...
  services.factory('Planboard', function ($rootScope, $resource, $q, $filter, $config, Log, StandBy, Store) {
    ...
    Planboard.prototype.availabilities = function () {
      ...
      try {
        _.each(unique, function (member) {
          queue.push(
            StandBy._('availability', { second: member.uuid, start: duration.start, end: duration.end }).then(function (results) {
              $rootScope.ticked();
    ...
    Planboard.prototype.clusters = function () {
      ...
      try {
        _.each(groups, function (group) {
          calls.push(
            StandBy._('cluster', { second: group.uuid, start: duration.start, end: duration.end }).then(function (results) {
              $rootScope.ticked();
